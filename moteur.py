        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "Create a professional looking mail for the user. "
                    "Just answer the mail in JSON with the format: { \"mail\": \"This is a mail\" }"
                )
            },
            {
                "role": "user",
...
def detect_audio_and_process(keys_to_press):
  ...
  result = generate_mail(transcription.text)
